  ...
  return (
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#fee4a9] to-[#f1b435]">
      ...
      <section className="flex flex-col md:flex-row justify-between items-center max-w-7xl mx-auto px-4 md:px-8 ">
        <div className="homepage-text max-w-2xl text-center md:text-left">
          ...
          <div className="space-y-4">
            ...
            <p className="text-xl md:text-2xl text-white ml-2 font-custom">
            En tu camino al éxito profesional 
            </p>
          </div>
        </div>
  
        <div className="homepage-img relative w-full md:w-[680px] overflow-hidden">
              ...
              {faqCards.slice(currentSlide, currentSlide + (window.innerWidth >= 768 ? 3 : 1)).map((card, index) => (
                <div key={index} className="flex-1 bg-amber-400 rounded-3xl p-6 md:p-8">
                  <div className="flex justify-between items-start mb-4 md:mb-6">
                    <h3 className="text-2xl font-bold text-navy-800 font-custom">{card.question}</h3>
  ...
  <div className="max-w-7xl mx-auto px-4 md:px-8">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
      {testimonials.map((testimonial, index) => (
        <div key={index} className="bg-white rounded-3xl overflow-hidden flex flex-col">
          <img 
            src={testimonial.image} 
            alt={testimonial.title}
            className="w-full h-48 md:h-64 object-cover"
          />
          <div className="p-6 md:p-8 flex flex-grow flex-col">
            ...
            <div className="h-full flex items-end justify-start"> {/* Align button to the left */}
              <button className="bg-navy-900 text-white px-3 py-1 md:px-4 md:py-2 rounded-full text-sm md:text-base hover:bg-navy-800 transition-colors">
                LEER MÁS
              </button>
            </div>
          </div>
